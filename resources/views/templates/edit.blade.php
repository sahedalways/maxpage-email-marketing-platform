@extends('layouts.core.frontend-classic')
@section('head')
...
@section('content')
    @if (auth()->user()->role == 'guest')
        <form id="classic-builder-form" action="{{ route('template.update', $template->uid) }}" method="POST"
            class="ajax_upload_form form-validate-jquery builder-classic-form">
            {{ csrf_field() }}
            ...
            <div class="row mr-0 ml-0">
                ...
                <div class="col-md-3 pr-0 pb-0 sidebar pr-4 pt-4 pl-4" style="overflow:auto;background:#f5f5f5">
                    ...
                    <hr>
                    @include('elements._tags', ['tags' => App\Models\Template::tags()])
                </div>
            </div>
        </form>
                    ...
                    editor.ui.registry.addMenuButton('acelletags', {
                        ...
                        fetch: function(callback) {
                            ...
                            items.push({
                                ...
                                onAction: function(_) {
                                    editor.insertContent(
                                        '<a href="{WEB_VIEW_URL}">Click to view web version of this email</a>'
                                    );
                                }
                            });

                            @foreach (App\Models\Template::tags() as $tag)
                                items.push({
                                    type: 'menuitem',
                                    text: '{{ $tag['name'] }}',
                                    onAction: function(_) {
