...
class Template extends Model
{
    ...
    public static function uploadTemplate($request, $asAdmin = false)
    {
        ...
        $validator = Validator::make($request->all(), $rules, [
            'file.mimetypes' => 'Please upload a valid .zip file',
            'uploaded' => "The file you try to upload exceed the server's max_upload_filesize of " . ini_get('upload_max_filesize'),
        ]);
        ...
        $attributes['customer_id'] = $request->user()->id;

        if (auth()->user()->role == 'guest') {
            $attributes['company_id'] = auth()->user()->id;
    ...
    public function findCssFiles($content)
    {
        // IMPORTANT
        // + No external CSS
        ...
        $document->loadHTML($content ?? $this->content, LIBXML_NOWARNING | LIBXML_NOERROR);
    ...
    public function getBuilderAdminTemplates()
    {
        ...
        foreach ($templates as $template) {
            $result[] = [
                'name' => $template->name,
                'url' => route('templates.builderChangeTemplate', ['uid' => $this->uid, 'change_uid' => $template->uid]),
    ...
    public function getPreviewContent($content = null)
    {
        ...
        $html = $pipeline->build()->process($content ?? $this->content);
    ...
    protected static function booted()
    {
        static::addGlobalScope('filterByRole', function (Builder $builder) {
            if (auth()->check()) {
                if (auth()->user()->role === 'guest') {
                    $builder->where('templates.is_guest', true);
                } else if (auth()->user()->role === 'company') {
                    $builder->where('templates.company_id', auth()->user()->id);
                } else {
                    $builder->where('templates.is_guest', false);
                }
            }
        });
    }
